...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Langue}.
 */
@RestController
@RequestMapping("/api")
public class LangueResource {
    ...
    public ResponseEntity<Langue> updateLangue(@PathVariable(value = "id", required = false) final Long id, @RequestBody Langue langue)
        ...
        Langue result = langueService.update(langue);
        return ResponseEntity
            .ok()
    ...
    public ResponseEntity<List<Langue>> getAllLangues(
        ...
        Page<Langue> page;
        if (eagerload) {
            page = langueService.findAllWithEagerRelationships(pageable);
        } else {
            page = langueService.findAll(pageable);
        }
