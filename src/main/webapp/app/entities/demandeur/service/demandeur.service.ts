...
type RestOf<T extends IDemandeur | NewDemandeur> = Omit<T, 'dateDeNaissance' | 'dateCreation'> & {
...
export class DemandeurService {
  ...
  protected convertDateFromClient<T extends IDemandeur | NewDemandeur | PartialUpdateDemandeur>(demandeur: T): RestOf<T> {
    return {
      ...demandeur,
      dateDeNaissance: demandeur.dateDeNaissance?.format(DATE_FORMAT) ?? null,
      dateCreation: demandeur.dateCreation?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDemandeur: RestDemandeur): IDemandeur {
    return {
      ...restDemandeur,
      dateDeNaissance: restDemandeur.dateDeNaissance ? dayjs(restDemandeur.dateDeNaissance) : undefined,
      dateCreation: restDemandeur.dateCreation ? dayjs(restDemandeur.dateCreation) : undefined,
    };
  }
